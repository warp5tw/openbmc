...
syspublickey=/publickey-root

verifypublickey() {
	if [ -f $syspublickey ];then
		if [[ -f publickey && -f publickey.sig ]];then
			sigfile=${1}.sig
			r=$(openssl dgst -verify $syspublickey  -sha256 -signature $sigfile  $1)
			echo $r
			rm $sigfile
		elif [ -f publickey.sig ];then
			echo "No publickey"
		else
			echo "No $sigfile"
		fi
	else
		echo "No $syspublickey"
	fi
}
...
verifysig() {
	sigfile=${1}.sig
	if [ -f $sigfile ];then
		r=$(openssl dgst -verify publickey  -sha256 -signature $sigfile  $1)
		rm $sigfile
		echo $r
	else
		echo "No $sigfile file"
	fi
}
...
if test -n "$doflash"
then
	echo "Need to flash images, so copy verify tool and verify publickey:$rorwopts"

	mkdir -p $rodir
	mount $rodev $rodir -t $(probe_rofs_type $rodev)
	...
	if [ "$publickey_result" != "Verified OK" ]; then
		echo "Verify publicKey Result:${publickey_result}"
		rm image-*
		doflash=
		imglist=
	else
		echo "publicKey verified OK"
	fi
fi
